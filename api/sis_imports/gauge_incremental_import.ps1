...
$token = "" # Gauge API Token
$url = "school.gauge-iad-prod.instructure.com" # Gauge URL without any path and do NOT include https://
...
#################################################
$api_url = "https://$url/api/sis_imports"

# Just in case $source_path or $output_path doesn't end with a \, add it.
if(!($source_path.EndsWith('\'))){
    $source_path += "\"
    Write-Host "You source_path didn't end with a \ so I added one.  It really is important"
}
if(!($output_path.EndsWith('\'))){
    $output_path += "\"
    Write-Host "You output_path didn't end with a \ so I added one.  It really is important"
}
...
$headers = @{'Accept'='*/*';"Authorization"="Token $token";"Content-Length"=$form_length}
